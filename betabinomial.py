...
df_gen  = pd.read_csv("donorFreq.txt", sep='\t', header=(0), dtype=float)
df_genR = pd.read_csv("recipientFreq.txt", sep='\t', header=(0), dtype=float)
...
df_gen['variantfreq_R'] = df_gen
df_gen['variantfreq_D'] = df_genR
...
for i in range(df_gen['variantfreq_R'].shape[0]):
    if (df_gen['variantfreq_R'][0] > cutoff and df_gen['variantfreq_D'][0] < cutoff): df_gen = df_gen.drop(df_gen.index[i])
...
plt.savefig('approximate_bottleneck_inference.pdf')
plt.show()